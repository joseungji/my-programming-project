...
# 0. Matplotlib 한글 폰트 설정 (Streamlit Cloud 대응) — 모든 그래프 생성보다 먼저 실행
...
def _init_korean_font():
    """OS별 한글 폰트를 감지/다운로드하여 matplotlib 전역 설정에 반영한다."""
    ...
    if system == "Windows":
        ...
        # Linux (Streamlit Cloud) — NanumGothic 폰트 확보 및 등록
        font_dest = os.path.join("/tmp", "NanumGothic.ttf")

        # 1) 이미 다운로드된 폰트 확인
        if not (os.path.exists(font_dest) and os.path.getsize(font_dest) > 1000):
            # 2) 시스템에 apt로 설치된 폰트를 /tmp로 복사
            for search_dir in [
                ...
                if os.path.isdir(search_dir):
                    ttf_files = sorted([
                        ...
                        if f.lower().endswith(".ttf") and "nanum" in f.lower() and "gothic" in f.lower()
                    ])
                    if ttf_files:
                        src = os.path.join(search_dir, ttf_files[0])
                        try:
                            _shutil.copy2(src, font_dest)
                        except Exception:
                            pass
                        break

        # 3) 그래도 없으면 GitHub에서 직접 다운로드
        if not (os.path.exists(font_dest) and os.path.getsize(font_dest) > 1000):
            download_urls = [
            ...
            for url in download_urls:
                try:
                    _urllib_request.urlretrieve(url, font_dest)
                    if os.path.exists(font_dest) and os.path.getsize(font_dest) > 1000:
                        break
                except Exception:
                    continue

        # 4) fontManager에 등록하고 sans-serif 폴백 리스트 맨 앞에 추가
        if os.path.exists(font_dest) and os.path.getsize(font_dest) > 1000:
            try:
                fm.fontManager.addfont(font_dest)
                prop = fm.FontProperties(fname=font_dest)
                font_name = prop.get_name()

                # font.family + sans-serif 폴백 리스트 모두 설정 (가장 확실한 방법)
