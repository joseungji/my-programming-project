...
# 배경음악 재생 (플레이리스트 - base64 inline, Streamlit Cloud 호환)
import streamlit.components.v1 as components
import base64 as _base64

# static/assets 또는 assets 폴더에서 MP3 파일 목록을 읽어옴
assets_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "assets")
if not os.path.exists(assets_dir):
    assets_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "assets")

playlist_names = []   # [파일명, ...]
playlist_b64 = []     # [base64문자열, ...]
if os.path.exists(assets_dir):
    try:
        mp3_files = sorted([f for f in os.listdir(assets_dir) if f.lower().endswith(".mp3")])
        for mp3_file in mp3_files:
            file_path = os.path.join(assets_dir, mp3_file)
            with open(file_path, "rb") as _f:
                playlist_b64.append(_base64.b64encode(_f.read()).decode("utf-8"))
            playlist_names.append(mp3_file)
    except Exception:
        pass
...
if playlist_b64:
    names_js = ", ".join([f'"{n}"' for n in playlist_names])
    data_js = ", ".join([f'"{d}"' for d in playlist_b64])
    ...
    components.html(
        ...
        <style>
            ...
            .bgm-wrap {{
                ...
                padding:6px 12px; background:rgba(255,255,255,0.7); border-radius:10px;
                border:1px solid #e0e0e0; max-width:340px; font-family:sans-serif;
            }}
            .bgm-icon {{ font-size:22px; cursor:pointer; user-select:none; transition:transform 0.15s; }}
            .bgm-icon:hover {{ transform:scale(1.2); }}
            .bgm-icon.playing {{ color:#ff6b6b; }}
            .bgm-vol {{ width:70px; accent-color:#ff6b6b; cursor:pointer; }}
            .bgm-btn {{
                border:none; border-radius:6px; background:#ff6b6b; color:white;
                font-size:14px; cursor:pointer; padding:4px 8px; transition:background 0.2s;
            }}
            .bgm-btn:hover {{ background:#e55a5a; }}
            .bgm-btn.muted {{ background:#999; }}
            .bgm-btn.muted:hover {{ background:#777; }}
            .bgm-title {{
                font-size:12px; color:#333; white-space:nowrap; overflow:hidden;
                text-overflow:ellipsis; max-width:100px; min-width:40px;
            }}
        </style></head><body>
        <div class="bgm-wrap">
            <span class="bgm-icon playing" id="icon" title="재생/일시정지">🎵</span>
            <span class="bgm-title" id="trackTitle">🎶</span>
            <input type="range" class="bgm-vol" id="vol" min="0" max="100" value="35" title="음량">
            <button class="bgm-btn" id="muteBtn" title="음악 켜기/끄기">🔊</button>
            <button class="bgm-btn" id="prevBtn">⏮</button>
            <button class="bgm-btn" id="nextBtn">⏭</button>
        </div>
        ...
        <script>
            ...
            function shortName(n){{let s=n.replace(/\.mp3$/i,'');return s.length>15?s.substring(0,13)+'..':s;}}
            const names=[{names_js}];
            const data=[{data_js}];
            let idx=0;
            function load(i){{
                idx=i;
                audio.src='data:audio/mpeg;base64,'+data[i];
                audio.load();
                titleEl.textContent=shortName(names[i]);
            }}
            load(0); audio.volume=0.35;
            audio.addEventListener('ended',()=>{{
                idx=(idx+1)%data.length; load(idx);
                audio.play().catch(()=>{{}});
            }});
            audio.play().then(()=>icon.classList.add('playing')).catch(()=>icon.classList.remove('playing'));
            icon.addEventListener('click',()=>{{
                if(audio.paused){{ audio.play().then(()=>icon.classList.add('playing')); }}
                else{{ audio.pause(); icon.classList.remove('playing'); }}
            }});
            prevBtn.addEventListener('click',()=>{{
                idx=(idx-1+data.length)%data.length; load(idx);
                audio.play().then(()=>icon.classList.add('playing'));
            }});
            ...
            muteBtn.addEventListener('click',()=>{{
                ...
                if(audio.muted){{ muteBtn.textContent='🔇'; muteBtn.classList.add('muted'); }}
                else{{ muteBtn.textContent='🔊'; muteBtn.classList.remove('muted'); }}
            }});
            audio.addEventListener('play',()=>icon.classList.add('playing'));
            audio.addEventListener('pause',()=>icon.classList.remove('playing'));
        </script></body></html>
        """,
        height=48,
    )
